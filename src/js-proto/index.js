...
class AnonApiRequest {
  constructor() {
    // Auth header will be added at call time. Headers are chosen to match android app. Modified JSON parsing to handle non-standard JSON intended to cause errors (we are no longer seeing this but we will keep it for now).
    this._axiosInstance = axios.create({ baseURL: config.anonymousBaseUrl, headers: { 'user-agent': config.userAgent, 'X-API-Key': config.anonymousApiKey, accept: 'application/json', 'content-type': 'application/json' }, transformResponse: (res) => JSON.parse(res.replace(/^\[\](?=\[)/, '')) });
...
class AnonApiData {
  constructor() {
    ...
    this.getAll = async (lat = 49.279844999999995, lon = -123.10200666666667) => {
      ...
      */
       ...
       */
      const vehicles = await anonApiRequest.get(`/availableVehicles/${cities[0].id}`, { "user-lat": lat, "user-lon": lon });
...
const main = async () => {
  ...
  try {
    const [models,options,parking,homezones,cities,vehicles] = await anonApiData.getAll();
